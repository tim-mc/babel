...
export function buildPrivateNamesMap(paths) {
  ...
  for (const path of paths) {
    if (path.isPrivate()) {
      ...
      privateNamesMap.set(name, {
        ...
        method: path.isClassPrivateMethod(),
        methodId: path.isClassPrivateMethod()
...
export function buildPrivateNamesNodes(privateNamesMap, loose, state) {
  ...
  for (const [
    ...
    // (the WeakMap), because static fields are directly assigned to a variable
...
export function buildFieldsInitNodes(
  ...
  return { staticNodes, instanceNodes, declarationNodes };
}
